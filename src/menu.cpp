...
void menu::refresh_room_win(ui::Window &room_win) {
    room_win.draw();
...
void menu::search_double_player_game(ui::Window &game_rooms_win, ui::Window &menu_win) {
    ...
    std::thread t = std::thread([&game_rooms_win, bc_receiver] {
        ...
        while (is_searching_game_rooms) {
            ...
            game_rooms_win.menu_items.emplace_front(
                    ui::MenuItem{
                            room_message.name() + " " + std::to_string(room_message.id()),
                            game_rooms_win.absolute_row(3 + game_rooms_win.menu_items.size()),
                            game_rooms_win.absolute_col(2),
                            nullptr
                    }
                    );
...
void menu::create_double_player_game(ui::Window &room_win) {
    ...
    room_message.set_id(utils::random_int(1000, 9999));
